...
For each manuscript, the code used to generate the figures is located in the subfolder `figures/`, labeled under the relevant manuscript name. The `landscape` folder indicates the integrative analysis that incorporates data from all tissues. Each topic sub-analysis group is responsible for the relevant code for their figures. If you have any questions or issues that show up, please [submit a new issue](hhttps://github.com/MoTrPAC/MotrpacPreSuspensionAcute/issues){target="_blank"} 
