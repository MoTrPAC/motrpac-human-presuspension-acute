...
```
<figure folder>/sources/
```

for example `figure_6/sources/`.
...
| ED3D, `extended_data_3/celltype_da_fit.R` | `CIBERSORTx_PrecovidBlood_RNADecon_Results_081226.csv` | `ED3D_CIBERSORTX_CSV`, `CELLTYPE_DA_CIBERSORTX_CSV` |
| ED2E | `landscape_figureED2_PTMSigDB_ORA_results_v2_Aug2026.csv` | `ED2E_PTMSIGDB_ORA_CSV` |
| FIG5B-FIG5H, ED7A, ED7B, ST5a | `Precovid_DEG_HOMER_KnownTF_Results/` (22 files) | none |
| FIG5C-FIG5H, ED7A, ED7B, ST5a | `tfproanno.RDS` | none |
| FIG6D, ED8A, ED8E, ST6c | `Gambardella-et-al-2020-table-s2.xlsx`, `TFEB_TARGET_GENES.v2024.1.Hs.tsv`, `GSM2354032_TFEB_e5_peaks.bed` | none |
| FIG6D, FIG6E, ED8A, ST6c | `TFEB_targets_v2.xlsx` | none |
...
**Two of the files above are not in this repository.** The ED1A genotype PCA
...
ED3D or ED3E, or running `extended_data_3/celltype_da_fit.R`, needs an approved consortium data-access
...
Figure 5's two inputs take no override, and `figure_5/ST5.R` reads them from `figure_5/sources/`
...
under `figure_5/sources/` and ED7 reads them from there.
...
| Raw RSEM gene counts | `extended_data_3/sex_da_fit.R`, `extended_data_3/celltype_da_fit.R` | `outputs/fits/*/raw-files/` |
...
| The clinical x omics fit | ED4G, FIG3EFG, EDT1 | `outputs/fits/clinical_omics/` (`figure_3/clinical_omics_fit.R`) |
...
`EPIGEN_QC_DIR` is deliberately one variable for all three panels and SF1A-SF1F. They read the same
...
place motrpac-human-presuspension-repro puts its bucket downloads, so a plain `Rscript figures/landscape/figure_1/ED1.R` fills it without setup.
...
**Fitted by `extended_data_3/sex_da_fit.R`** into `outputs/fits/sex_da/`, under
...
`extended_data_3/sex_da_fit.R` records the model, the inputs and the package versions per table in
...
ED3D plots these fractions and `extended_data_3/celltype_da_fit.R` fits ED3E's table on them, and
...
**Fitted by `extended_data_3/celltype_da_fit.R`** into `outputs/fits/celltype_da/`.
...
version the table was fitted *from*, so a table fitted here against the v2.0 collection objects cannot
...
| `named_features` | none | ED1C and ST1e | `motrpac_named_features.csv` |
| `pathways` | `FIG4A`, `FIG4B` | FIG4A, FIG4B | `Curated_multi-tissue_10.3.2025.xlsx`, sheet `rna` (precovid-analyses PR #64); FIG4B read off the assembled panel |
...
## FIG3EFG, ED4G, EDT1, ST3g and ST3h: the clinical x omics fit

Not a file that ships with the repo and not one you supply: **`figure_3/clinical_omics_fit.R` fits it**, into
...
`FIG3_CLINICAL_OMICS_TABLE` points FIG3EFG and ED4G (not EDT1, ST3g or ST3h) at a fit produced elsewhere and makes the fit a
...
Not files that ship with the repo and not ones you supply: **`figure_4/plier_fit.R` fits them**, into
...
`figure_4/plier_lvs.env` exists for and what its header explains: the LV numbers there were read off the
legacy's fit, they are not derived from anything, and they hold because `figure_4/plier_fit.R`
...
count other than one is an error; `supplementary_figure_1/SF1.R` holds the resolver. Read by SF1C-SF1F.
...
Six files under `figure_6/sources/`, all from precovid-analyses PR #104
...
against `hg19_basicgenes`, bound = a promoter or 1-5 kb peak). Settembre et al. 2013 adds two
genes by ChIP-qPCR, coded in `figure_6/FIG6_ED8_helpers.R`. `tfeb_chip_evidence()` scores any
...
above 0.1, the merge of the four networks (muscle-only and blood-to-muscle, EE and RE) in Cytoscape,
...
gene lists, one per tissue x arm x timepoint and thresholded on nominal p, were written out of the
...
`figure_5/FIG5_ED7_helpers.R` is the only link between a file on disk
...
added. The helper takes the best score over the four locations per gene symbol. precovid-analyses PR #106 fetched
the full file at run time; the channel is rebuilt without a version, so the figure is pinned to this copy.
